...
async function handleMessage(request: WorkerRequest): Promise<void> {
  try {
    switch (request.type) {
      ...
      case 'initWithMnemonic': {
        await loadLibraries();
        if (!libs) throw new Error('Libraries not loaded');

        const rawMnemonic = request.mnemonic.trim().toLowerCase();
        if (!libs.bip39.validateMnemonic(rawMnemonic)) {
          throw new Error('Frase de recuperação inválida');
        }

        await initializeFromMnemonic(rawMnemonic);
        respond({ id: request.id, type: 'initWithMnemonicResult', success: true });
        break;
      }
