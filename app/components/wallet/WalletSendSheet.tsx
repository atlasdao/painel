...
import { X, ArrowRight, AlertTriangle, Check, Copy, Camera, Loader2, Eye, EyeOff, Bitcoin, Fingerprint, AtSign, User } from 'lucide-react';
...
export default function WalletSendSheet({ isOpen, onClose, balances, utxos, onTxSent, biometricEnabled, onBiometricAuth }: WalletSendSheetProps) {
  ...
  // @username resolution
  const [resolvedUser, setResolvedUser] = useState<{ username?: string; address?: string; found: boolean } | null>(null);
  const [resolving, setResolving] = useState(false);

  const isLiquidAddress = (s: string) => /^(lq1|VJL|ex1|CT|ej)/i.test(s);
  ...
  useEffect(() => {
    ...
    if (isLiquidAddress(recipientInput)) {
      setAddress(recipientInput);
      setResolvedUser(null);
      return;
    }
    if (recipientInput.startsWith('@') && recipientInput.length > 1) {
      const timer = setTimeout(async () => {
        setResolving(true);
        try {
          const result = await transferService.resolve(recipientInput);
          if (result.found && result.address) {
            setResolvedUser({ username: result.username, address: result.address, found: true });
            setAddress(result.address);
          } else {
            setResolvedUser({ found: false });
            setAddress('');
          }
        } catch {
          setResolvedUser({ found: false });
          ...
          <div className="p-4 space-y-4">
            ...
            {step === 'input' && (
              <>
                ...
                {/* Address input */}
                <div>
                  <label className="text-xs text-[var(--text-muted)] mb-1.5 block">Endereco ou @username</label>
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      {recipientInput.startsWith('@') && (
                        <AtSign size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--accent)' }} />
                      )}
                      <input
                        type="text"
                        value={recipientInput}
                        onChange={e => setRecipientInput(e.target.value)}
                        placeholder="lq1..., VJL... ou @usuario"
                        className="w-full px-3 py-2.5 bg-[var(--bg-tertiary)] border border-[var(--border-default)] rounded-xl text-sm font-mono text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)]"
                        style={recipientInput.startsWith('@') ? { paddingLeft: '2rem' } : {}}
                        autoComplete="off"
                      />
                      {resolving && (
                        <Loader2 size={14} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin" style={{ color: 'var(--accent)' }} />
                      )}
                    </div>
                    <button
                  ...
                  {resolvedUser?.found && (
                    <div className="mt-2 flex items-center gap-2 p-2 rounded-lg" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)' }}>
                      <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: 'var(--bg-elevated)' }}>
                        <User size={14} style={{ color: 'var(--text-secondary)' }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>@{resolvedUser.username}</span>
                        <span className="text-xs ml-2 font-mono" style={{ color: 'var(--text-muted)' }}>
                          {resolvedUser.address?.slice(0, 8)}...{resolvedUser.address?.slice(-6)}
                        </span>
                      </div>
                      <Check size={14} style={{ color: 'var(--color-success)' }} />
                    </div>
                  )}
                  {resolvedUser && !resolvedUser.found && !resolving && (
                    <p className="mt-1 text-xs" style={{ color: 'var(--color-error)' }}>Usuario nao encontrado</p>
                  )}
                </div>
