...
import { ArrowDownLeft, ArrowUpRight, Clock, ArrowLeftRight, Banknote, Users } from 'lucide-react';

interface WalletActionsProps {
  onReceive: () => void;
  onSend: () => void;
  onAtlasTransfer: () => void;
  onHistory: () => void;
  disabled?: boolean;
}

function WalletActionsInner({ onReceive, onSend, onAtlasTransfer, onHistory, disabled }: WalletActionsProps) {
  return (
    <div className="grid grid-cols-6 gap-1 w-full max-w-md mx-auto">
      ...
      <button
        onClick={onAtlasTransfer}
        disabled={disabled}
        className="flex flex-col items-center gap-1 py-2 group disabled:opacity-50"
      >
        <div className="w-11 h-11 bg-violet-500/15 rounded-full flex items-center justify-center group-hover:bg-violet-500/25 transition-colors">
          <Users className="w-5 h-5 text-violet-500" />
        </div>
        <span className="text-[10px] font-medium text-[var(--text-secondary)]">Atlas</span>
      </button>
