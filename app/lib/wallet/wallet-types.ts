...
// PIN-encrypted wallet blob stored in localStorage
export interface PinEncryptedBlob {
  ciphertext: string;  // hex
  iv: string;          // hex
  salt: string;        // hex
  deviceSalt: string;  // hex
  iterations: number;  // 100000
}

// PIN security settings stored per user
...
export type WorkerRequest =
  ...
  | { id: string; type: 'initWithMnemonic'; mnemonic: string }
...
export type WorkerResponse =
  ...
  | { id: string; type: 'initWithMnemonicResult'; success: boolean }
