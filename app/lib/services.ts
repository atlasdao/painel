...
export const adminService = {
  ...
  async getBackupSummary(): Promise<any> {
    const response = await api.get('/admin/backup/summary');
    return response.data;
  },
  ...
  async createBackup(data: {
    ...
    const response = await api.post('/admin/backup/create', data);
    return response.data;
  },
  ...
  async getBackupDownloadUrl(id: string): Promise<any> {
    const response = await api.get(`/admin/backup/${id}/download-url`);
    return response.data;
  },

  async getDatabaseTables(): Promise<{ name: string; rowCount: number }[]> {
  ...
  async toggleBackupSchedule(id: string): Promise<any> {
    const response = await api.patch(`/admin/backup/schedules/${id}/toggle`);
    return response.data;
  },

  async deleteBackupSchedule(id: string): Promise<any> {
...
export const profileService = {
  ...
  async getTrustScore(): Promise<any> {
    const response = await api.get('/profile/trust-score');
    return response.data;
  },

  async registerAddress(data: { chain: string; address: string; label?: string; isDefault?: boolean }): Promise<any> {
...
export const walletProxyService = {
  ...
  async exportForMigration(encryptedBlob: any) {
    const response = await api.post('/wallet/migrate/export', { encryptedBlob });
    return response.data;
  },
  async importFromMigration(token: string) {
    const response = await api.post('/wallet/migrate/import', { token });
    return response.data;
  },
};
...
// ==================== TRANSFER SERVICE (Atlas-to-Atlas) ====================
export const transferService = {
  async resolveRecipient(identifier: string): Promise<{
    id: string;
    username: string;
    atlasTag: string;
    ...
    const response = await api.post('/transfer/resolve', { identifier });
    return response.data;
  },
  ...
  async confirmTransfer(transferId: string, txid: string): Promise<any> {
    const response = await api.post(`/transfer/${transferId}/confirm`, { txid });
    return response.data;
  },
  ...
  // Resolve method used by WalletSendSheet and useUsernameResolver
  async resolve(identifier: string): Promise<any> {
    try {
      const data = await this.resolveRecipient(identifier);
      return {
        found: true,
        userId: data.id,
        username: data.username || data.atlasTag,
        atlasTag: data.atlasTag,
        profilePicture: data.profilePicture,
        address: data.defaultAddress?.address,
        addresses: data.defaultAddress ? [data.defaultAddress] : [],
      };
    } catch {
      return { found: false, userId: '', username: '', addresses: [] };
    }
  },
};