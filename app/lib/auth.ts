...
export const authService = {
  async login(email: string, password: string): Promise<AuthResponse | { requiresTwoFactor: boolean; sessionToken: string; user?: { email: string } }> {
    ...
    const userData = {
      ...
      // v2 fields
      atlasTag: user.atlasTag || null,
      walletVersion: user.walletVersion || undefined,
      onboardingV2Completed: user.onboardingV2Completed || false,
      preferredChain: user.preferredChain || undefined,
      seedBackedUp: user.seedBackedUp || false,
  ...
  async setUsername(username: string): Promise<void> {
    await api.patch('/auth/set-username', { username });
  },

  async register(email: string, password: string, options?: { username?: string; referralCode?: string }): Promise<AuthResponse> {
