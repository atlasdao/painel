  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authService.register(
        formData.email,
        formData.password,
        { username: formData.username, referralCode: referralCode || undefined }
      );
        ...
        {referralCode && (
          <div className="mb-6 p-4 rounded-lg bg-purple-500/10 border border-purple-500/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[var(--text-primary)] font-medium">Você foi convidado!</p>
                <p className="text-purple-400 text-sm">
                  Ao se cadastrar, você entra no programa de indicações da Atlas.
                </p>
              </div>
            </div>
          </div>
