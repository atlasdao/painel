...
export class PixController {
  ...
  @ApiParam({ name: 'transactionId', description: 'Transaction ID to check status' })
  ...
  @ApiResponse({ status: 200, description: 'Eulen API is healthy and responding' })
  @ApiResponse({ status: 401, description: 'Unauthorized - invalid JWT or API key' })
  @ApiResponse({ status: 503, description: 'Eulen API is unavailable or not responding' })
  async ping() {
    return this.pixService.pingEulenAPI();
  }
