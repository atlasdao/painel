...
describe('PixService', () => {
	...
	beforeEach(async () => {
		...
		eulenClient = module.get(EulenClientService) as jest.Mocked<EulenClientService>
	...
	describe('createDeposit', () => {
		...
		it('should throw BadRequestException for invalid amount', async () => {
			...
			await expect(service.createDeposit(mockUser.id, depositDto)).rejects.toThrow(BadRequestException)
		})
	})
	...
	describe('createWithdraw', () => {
		...
		it('should throw ForbiddenException for insufficient balance', async () => {
			...
			await expect(service.createWithdraw(mockUser.id, withdrawDto)).rejects.toThrow(ForbiddenException)
		})
	})
	...
	describe('getTransactions', () => {
		it('should get user transactions with DePix addresses', async () => {
			...
			expect(transactionRepository.findByUserId).toHaveBeenCalledWith(mockUser.id, {})
		})
		...
		it('should filter transactions by status', async () => {
			...
			transactionRepository.findByUserId.mockResolvedValue([completedTransaction])
	...
	describe('getTransactionById', () => {
		it('should get transaction by ID', async () => {
			...
			const result = await service.getTransactionById(mockUser.id, mockTransaction.id)
		...
		it('should throw NotFoundException for non-existent transaction', async () => {
			...
			await expect(service.getTransactionById(mockUser.id, 'invalid-id')).rejects.toThrow(NotFoundException)
		})
		...
		it('should throw ForbiddenException for unauthorized access', async () => {
			...
			await expect(service.getTransactionById(mockUser.id, mockTransaction.id)).rejects.toThrow(ForbiddenException)
		})
	})
	...
	describe('getTransactionStatus', () => {
		it('should get transaction status from Eulen for DePix', async () => {
			...
			const result = await service.getTransactionStatus(mockUser.id, mockTransaction.id)
	...
	describe('validatePixKey', () => {
		it('should validate real DePix address', async () => {
			...
			expect(eulenClient.validatePixKey).toHaveBeenCalledWith(REAL_DEPIX_ADDRESS)
		})
	...
	describe('updateTransactionFromEulen', () => {
		...
		it('should handle Eulen API errors during update', async () => {
			transactionRepository.findById.mockResolvedValue(mockTransaction)
			eulenClient.getDepositStatus.mockRejectedValue(new Error('Eulen API error'))

			// Should not throw, just log the error
			await expect(service.updateTransactionFromEulen(mockTransaction.id)).resolves.not.toThrow()
		})
	})
})