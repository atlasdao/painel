...
export class AccountValidationController {
	...
	@ApiResponse({
		status: 200,
		description: 'Returns user limits and reputation',
	})
	async getUserLimits(@Req() req: any) {
		...
		return this.accountValidationService.getUserLimitsAndReputation(
			req.user.id,
		);
	}
	...
	@ApiResponse({
		status: 200,
		description: 'Returns detailed validation debug info',
	})
	async getDebugValidationStatus(@Req() req: any) {
		...
		return this.accountValidationService.getDetailedValidationStatus(
			req.user.id,
		);
	}
	...
	@ApiResponse({
		status: 200,
		description: 'Manual validation check triggered',
	})
