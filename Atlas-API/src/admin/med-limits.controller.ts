...
export class MedLimitsController {
	...
	async updateMedLimits(@Body() dto: UpdateMedLimitsDto) {
		try {
			...
			if (
				dto.monthlyDepositLimit !== undefined &&
				dto.monthlyDepositLimit <= 0
			) {
				throw new HttpException(
					'Monthly deposit limit must be positive',
					HttpStatus.BAD_REQUEST,
				);
			}
			if (
				dto.monthlyWithdrawLimit !== undefined &&
				dto.monthlyWithdrawLimit <= 0
			) {
				throw new HttpException(
					'Monthly withdraw limit must be positive',
					HttpStatus.BAD_REQUEST,
				);
			}
			if (
				dto.maxTransactionAmount !== undefined &&
				dto.maxTransactionAmount <= 0
			) {
				throw new HttpException(
					'Max transaction amount must be positive',
					HttpStatus.BAD_REQUEST,
				);
			}
			if (dto.firstDayLimit !== undefined && dto.firstDayLimit <= 0) {
				throw new HttpException(
					'First day limit must be positive',
					HttpStatus.BAD_REQUEST,
				);
			}
