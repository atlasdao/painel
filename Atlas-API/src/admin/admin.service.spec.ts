...
describe('AdminService', () => {
	...
	describe('getAllUsers', () => {
		...
		it('should filter active users', async () => {
			...
			expect(userRepository.findActiveUsers).toHaveBeenCalledWith({ isActive: true })
		})
		...
		it('should filter inactive users', async () => {
			...
			expect(userRepository.findActiveUsers).toHaveBeenCalledWith({ isActive: false })
		})
		...
		it('should apply pagination parameters', async () => {
			...
			expect(userRepository.findAll).toHaveBeenCalledWith({ skip: 10, take: 20 })
		})
	})
	...
	describe('updateUserStatus', () => {
		it('should activate user successfully', async () => {
			...
			expect(userRepository.update).toHaveBeenCalledWith('user-123', { isActive: true })
		})
		...
		it('should throw NotFoundException for non-existent user', async () => {
			...
			await expect(service.updateUserStatus('invalid-id', true)).rejects.toThrow(NotFoundException)
		})
	})
	...
	describe('revokeUserApiKey', () => {
		it('should revoke API key successfully', async () => {
			...
			expect(userRepository.update).toHaveBeenCalledWith('user-123', { apiKey: null })
		})

		it('should throw NotFoundException for non-existent user', async () => {
			userRepository.findById.mockResolvedValue(null)

			await expect(service.revokeUserApiKey('invalid-id')).rejects.toThrow(NotFoundException)
		})
	})
	...
	describe('deleteUser', () => {
		it('should delete user without pending transactions', async () => {
			...
			expect(transactionRepository.findByUserId).toHaveBeenCalledWith('user-123', { status: 'PENDING' })
		...
		it('should throw ForbiddenException if user has pending transactions', async () => {
			...
			userRepository.findById.mockResolvedValue(mockUser)
			transactionRepository.findByUserId.mockResolvedValue([pendingTransaction])

			await expect(service.deleteUser('user-123')).rejects.toThrow(ForbiddenException)
	...
	describe('getUserStats', () => {
		it('should get user statistics', async () => {
			...
			userRepository.count.mockResolvedValueOnce(50) // total
	...
	describe('updateTransactionStatus', () => {
		it('should update transaction status to COMPLETED', async () => {
			const updatedTransaction = { ...mockTransaction, status: 'COMPLETED', completedAt: new Date() }
			...
			const result = await service.updateTransactionStatus('trans-123', 'COMPLETED')
		...
		it('should update transaction status to FAILED with error message', async () => {
			const failedTransaction = { ...mockTransaction, status: 'FAILED', errorMessage: 'Payment declined' }
			transactionRepository.findById.mockResolvedValue(mockTransaction)
			transactionRepository.update.mockResolvedValue(failedTransaction)

			const result = await service.updateTransactionStatus('trans-123', 'FAILED', 'Payment declined')
		...
		it('should throw NotFoundException for non-existent transaction', async () => {
			...
			await expect(service.updateTransactionStatus('invalid-id', 'COMPLETED')).rejects.toThrow(NotFoundException)
		})
	})
	...
	describe('getSystemStats', () => {
		it('should get comprehensive system statistics', async () => {
			...
			jest.spyOn(service, 'getTransactionStats').mockResolvedValue(transactionStats)
