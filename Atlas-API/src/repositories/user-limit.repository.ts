...
export class UserLimitRepository extends AbstractBaseRepository<UserLimit> {
  ...
  async getOrCreateUserLimits(userId: string): Promise<UserLimit> {
    ...
    if (!limits) {
      ...
      limits = await this.prisma.userLimit.create({
        data: {
          userId,
          dailyDepositLimit: 500.00,    // R$ 500 first day
          dailyWithdrawLimit: 500.00,
          dailyTransferLimit: 500.00,
          maxDepositPerTx: 5000.00,     // R$ 5000 per transaction
          maxWithdrawPerTx: 5000.00,
          maxTransferPerTx: 5000.00,
          monthlyDepositLimit: 50000.00, // R$ 50k monthly
  ...
  async updateUserLimits(
    userId: string,
    updates: Partial<Omit<UserLimit, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>,
    adminId: string,
  ): Promise<UserLimit> {
  ...
  async getAllUsersWithLimits(params?: {
    ...
    users: (UserLimit & { user: { email: string; username: string; createdAt: Date } })[];
    total: number;
  }> {
