...
export class ApiKeyRequestController {
  ...
  @ApiResponse({ status: 409, description: 'Conflict - User already has request or API key' })
  async createRequest(
    @Req() req: any,
  ...
  @ApiQuery({ name: 'status', required: false, enum: ['PENDING', 'APPROVED', 'REJECTED', 'REVOKED'] })
