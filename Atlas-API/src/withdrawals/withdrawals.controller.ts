...
export class WithdrawalsController {
	...
	async getAllWithdrawals(
		...
		console.log(
			`🔍 Admin getAllWithdrawals called by user: ${req.user.id}, role: ${req.user.role}`,
		);
	...
	@Get('admin/pending')
	@ApiOperation({
		summary: 'Get pending withdrawals ready for processing (Admin)',
	})
	@ApiResponse({ status: 200, description: 'Returns pending withdrawals' })
	@ApiResponse({ status: 403, description: 'Forbidden - Admin only' })
	async getPendingWithdrawals(@Request() req) {
		console.log(
			`🔍 Admin getPendingWithdrawals called by user: ${req.user.id}, role: ${req.user.role}`,
		);
	...
	@ApiResponse({
		status: 200,
		description: 'Returns system withdrawal statistics',
	})
	@ApiResponse({ status: 403, description: 'Forbidden - Admin only' })
	async getSystemWithdrawalStats(@Request() req) {
		console.log(
			`🔍 Admin getSystemWithdrawalStats called by user: ${req.user.id}, role: ${req.user.role}`,
		);
	...
	async approveOrRejectWithdrawal(
		...
		return this.withdrawalsService.approveOrRejectWithdrawal(
			id,
			req.user.id,
			dto,
		);
	...
	async approveWithdrawal(
		@Request() req,
		@Param('id') id: string,
		@Body() dto: { adminNotes?: string; coldwalletTxId?: string },
	) {
		if (req.user.role !== UserRole.ADMIN) {
			throw new HttpException('Acesso negado', HttpStatus.FORBIDDEN);
		}
	...
	async rejectWithdrawal(
		@Request() req,
		@Param('id') id: string,
		@Body() dto: { adminNotes?: string; rejectionReason?: string },
	) {
		if (req.user.role !== UserRole.ADMIN) {
			throw new HttpException('Acesso negado', HttpStatus.FORBIDDEN);
		}
		return this.withdrawalsService.approveOrRejectWithdrawal(
			...
			req.user.id,
			{
				approve: false,
				adminNotes: dto.adminNotes,
				statusReason: dto.rejectionReason,
			},
		);
	}
