...
export class WithdrawalsService {
  ...
  private readonly PIX_FEE_PERCENTAGE = 0.01; // 1%
  private readonly DEPIX_FEE_FIXED = 3.50; // R$ 3.50
  ...
  async createWithdrawal(userId: string, dto: CreateWithdrawalDto) {
    ...
    if (!user || !user.isActive) {
      throw new HttpException('Usuário não encontrado ou inativo', HttpStatus.NOT_FOUND);
    }
    ...
    if (dto.method === WithdrawalMethod.PIX) {
      fee = dto.amount * this.PIX_FEE_PERCENTAGE;
      ...
      throw new HttpException('Método de saque inválido', HttpStatus.BAD_REQUEST);
    }
  ...
  async getWithdrawalById(id: string, userId?: string) {
    ...
    if (!withdrawal) {
      throw new HttpException('Solicitação de saque não encontrada', HttpStatus.NOT_FOUND);
    }
  ...
  // Admin methods
  async getAllWithdrawals(status?: WithdrawalStatus, method?: WithdrawalMethod) {
    const where: any = {};
  ...
  async approveOrRejectWithdrawal(
    ...
    if (dto.approve) {
      ...
      updateData.statusReason = dto.statusReason || 'Rejeitado pelo administrador';
    }
